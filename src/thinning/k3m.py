...
def k3m(binary):
    skeleton = np.pad(binary, 1, mode="constant", constant_values=0)
    ROW, COL = skeleton.shape

    while True:
        change = False

        borders = np.isin(
            correlate2d(skeleton, WINDOW, mode='same'),
            LOOKUP[0]
        ) & skeleton.astype(np.bool_)

        for phase in range(1, 6):
            for i in range(1, ROW-1):
                for j in range(1, COL-1):
                    if borders[i, j]:
                        weight = np.sum(WINDOW * skeleton[i-1:i+2, j-1:j+2])
